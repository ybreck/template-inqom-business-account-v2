...
import { ModuleComponentProps, SupplierPaymentBatch, SupplierPaymentStatus } from '../../../types';
import { mockSupplierPayments } from './data/supplierPayments';
import { mockBankAccounts } from '../ventes/data/quotes'; // Assuming shared bank accounts
import { MagnifyingGlassIcon, PlusIcon, PencilIcon } from '../../constants/icons';

interface SupplierPaymentListPageProps extends ModuleComponentProps {}
...
const getStatusBadgeStyle = (status: SupplierPaymentStatus): string => {
  switch (status) {
    case 'Payé':
...
const SupplierPaymentListPage: React.FC<SupplierPaymentListPageProps> = ({ onSubNavigate }) => {
  const [paymentBatches] = useState<SupplierPaymentBatch[]>(mockSupplierPayments);
  ...
  }, [paymentBatches, searchTerm]);

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-semibold text-theme-text">Paiements</h2>
        <button
          onClick={() => onSubNavigate?.('supplier_payment_editor?id=new')}
      ...
      <div className="space-y-3">
        {filteredPaymentBatches.map(batch => {
            ...
            return (
              <div
                key={batch.id}
                onClick={() => onSubNavigate?.(`supplier_payment_detail?id=${batch.id}`)}
                  ...
                  <div className="flex-1 min-w-0">
                      ...
                      <div className="text-right flex items-center space-x-2">
                        ...
                        {batch.status === 'En préparation' && (
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    onSubNavigate?.(`supplier_payment_editor?id=${batch.id}`);
                                }}
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-100 flex justify-between items-center">
                        <div className="flex flex-wrap items-center gap-2 text-xs">
                            ...
                            {batch.paymentDate && (
                                <span className="bg-theme-secondary-gray-100 text-theme-secondary-gray-700 px-2 py-1 rounded-md">
                                    Payé le <strong className="font-semibold">{formatDate(batch.paymentDate)}</strong>
                                </span>
                            )}
                        </div>
                    </div>
